function upyunPicUpload(mediaId,imgId,width,height,token){
	art.dialog.data('width', width);
	art.dialog.data('height', height);
	art.dialog.data('mediaId', mediaId); //use for update back the element value
	art.dialog.data('imgId', imgId); //use for update the image
	art.dialog.data('lastpic', $('#'+mediaId).val());
	art.dialog.open('/editor/upload/'+token+'?width='+width,{lock:true,title:'上传图片',width:600,height:400,yesText:'关闭',background: '#000',opacity: 0.45});
}
