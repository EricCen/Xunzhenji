...
<script>
    if (location.hash.indexOf('error') != -1) {
        ...
        if (ua.indexOf('micromessenger') != -1) {
            ...
            redirect("${net.xunzhenji.alipay.AlipayContext.ALIPAY_GATEWAY_URL}", {
                service: "${net.xunzhenji.alipay.AlipayContext.SERVICE_WAP_DIRECT_PAY}",
                partner: "${context.partner}",
                _input_charset: "${context.inputCharset}",
                sign_type: "${payment.signType}",
                sign: "${payment.sign}",
                notify_url: "${context.notifyUrl}",
                return_url: "${context.returnUrl}",
                out_trade_no: "${payment.outTradeNo}",
                subject: "${payment.subject}",
                total_fee: "${payment.cashFee}",
